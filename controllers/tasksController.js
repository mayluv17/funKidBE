...
const getAllTasks = async (req, res) => {
  ...
  const refactored = allTasks.map((task, i) => ({
    ...task._doc,
    id: i + 1,
    userCompleted: task._doc.userCompleted.length,
  }));
...
const getAllTasksBycategory = async (req, res) => {
  ...
  if (!taskInCategory)
    return res
      .status(204)
      .json({ message: 'No Tasks found in this category.' });

  const refactoredTaskInCategory = await taskInCategory.map((task) => ({
    ...task._doc,
    userCompleted: task.userCompleted.includes(req.userId),
  }));
...
const getSingleTask = async (req, res) => {
  ...
  const userTaskActions = await TaskData.find({
    taskId: taskId,
    userId: req.userId,
  });
  if (!taskInCategory)
    return res
      .status(204)
      .json({ message: 'This task is no longer availaible' });

  const refactoredTaskInCategory = await {
    ...taskInCategory._doc,
    userCompleted: taskInCategory._doc.userCompleted.includes(req.userId),
  };
  // console.log(userTaskActions[0].content[0].content)

  res.json({
    singleTaskData: refactoredTaskInCategory,
    userTaskActions: userTaskActions,
  });
};
...
const updateTask = async (req, res) => {
  ...
  if (!task) {
    return res
      .status(204)
      .json({ message: `No task matches ID ${req.body.taskId}.` });
  }
...
const deleteTask = async (req, res) => {
  if (!req?.body?.taskId)
    return res.status(400).json({ message: 'You need to select a task' });

  const task = await Tasks.findOne({ _id: req.body.taskId }).exec();
  if (!task) {
    return res
      .status(204)
      .json({ message: `No task matched ID ${req.body.taskId}.` });
  }
